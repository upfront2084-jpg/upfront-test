...
router.get('/reports/:type', ah(async (req, res) => {
  ...
  if (type === 'leads-por-periodo') {
    columns = ['Mês', 'Leads'];
    const raw = await all(`SELECT entry_date FROM leads WHERE entry_date BETWEEN ? AND ?${scopeSql}`, [from, to, ...params]);
    const map = {};
    ...
    rows = raw.map((r) => [r.source || 'Não informado', r.n]);
    ...
    rows = raw.map((r) => [r.source || 'Não informado', r.total, r.enrolled, r.total ? `${Math.round((r.enrolled / r.total) * 1000) / 10}%` : '0%']);
    ...
    rows = raw.map((r) => [r.date, r.lead_name, r.teacher_name || '—', r.status, r.result || '—']);
    ...
    rows = raw.map((r) => [r.enrollment_date, r.lead_name, r.package_name || '—', r.teacher_name || '—', r.monthly_value]);
    ...
    rows = raw.map((r) => [r.name, r.source || '—', (r.last_stage_change_at || '').slice(0, 10), daysBetween(todayISO(), r.last_contact_date)]);
    ...
    rows = raw.map((r) => [r.name, r.campaign, (r.responded_at || '').slice(0, 10), r.enrolled ? 'Sim' : 'Não']);
  } else if (type === 'taxa-recuperacao') {
    const totalSent = (await one(`SELECT COUNT(*) n FROM campaign_recipients`)).n;
    const responded = (await one(`SELECT COUNT(*) n FROM campaign_recipients WHERE responded = 1`)).n;
    const recovered = (await one(`SELECT COUNT(*) n FROM campaign_recipients WHERE enrolled = 1 OR interested = 1`)).n;
    ...
    const trialsDone = (await one(`SELECT COUNT(DISTINCT lead_id) n FROM trial_classes WHERE status='Realizada'`)).n;
    const enrolledAfterTrial = (await one(
    ...
    const totalLeads = (await one(`SELECT COUNT(*) n FROM leads WHERE entry_date BETWEEN ? AND ?${scopeSql}`, [from, to, ...params])).n;
    const enrolled = (await one(`SELECT COUNT(*) n FROM leads WHERE status='matriculado' AND entry_date BETWEEN ? AND ?${scopeSql}`, [from, to, ...params])).n;
    columns = ['Métrica', 'Valor'];
    rows = [

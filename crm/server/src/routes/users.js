...
router.put('/users/:id', requireRole('admin'), ah(async (req, res) => {
  ...
  if (b.username !== undefined) {
    ...
    if (!/^[a-z0-9._-]+$/.test(username)) return res.status(400).json({ error: 'Usuário deve ter só letras, números, ponto, hífen ou underline' });
    const clash = await one('SELECT id FROM users WHERE username = ? AND id != ?', [username, u.id]);
    if (clash) return res.status(409).json({ error: 'Já existe um usuário com esse nome de login' });
  }
  await run('UPDATE users SET name=?, username=?, role=?, teacher_id=?, active=?, updated_at=? WHERE id=?', [
...
router.put('/users/:id/password', requireRole('admin'), ah(async (req, res) => {
  ...
  await run('UPDATE users SET password_hash=?, updated_at=? WHERE id=?', [hashPassword(password), nowISO(), u.id]);
  res.json({ ok: true });
}));

async function deleteUserCascade(id, targetId) {
  await run('UPDATE leads SET owner_user_id = ? WHERE owner_user_id = ?', [targetId, id]);
  await run('UPDATE tasks SET assigned_user_id = ? WHERE assigned_user_id = ?', [targetId, id]);
  await run('UPDATE campaigns SET responsible_user_id = ? WHERE responsible_user_id = ?', [targetId, id]);
  await run('UPDATE interactions SET user_id = NULL WHERE user_id = ?', [id]);
  await run('UPDATE notes SET user_id = NULL WHERE user_id = ?', [id]);
...
router.delete('/users/:id', requireRole('admin'), ah(async (req, res) => {
  ...
  if (u.role === 'admin') {
    const otherAdmins = await one("SELECT COUNT(*) as n FROM users WHERE role = 'admin' AND active = 1 AND id != ?", [u.id]);
  ...
  let targetId = null;
  if (reassignToUserId) {
    const target = await one('SELECT id FROM users WHERE id = ? AND active = 1', [reassignToUserId]);
  ...
  await transaction(() => deleteUserCascade(u.id, targetId));
  res.json({ ok: true });
}));

router.post('/users/bulk-delete', requireRole('admin'), ah(async (req, res) => {
  ...
  const existingRows = await all(`SELECT id FROM users WHERE id IN (${placeholders})`, uniqueIds);
  const existingIds = existingRows.map((r) => r.id);
  if (!existingIds.length) return res.status(400).json({ error: 'Nenhum usuário válido selecionado' });

  const beingDeletedAreAdmins = (await one(`SELECT COUNT(*) as n FROM users WHERE role = 'admin' AND id IN (${placeholders})`, uniqueIds)).n > 0;
  if (beingDeletedAreAdmins) {
    const remainingAdmins = await one(`SELECT COUNT(*) as n FROM users WHERE role = 'admin' AND active = 1 AND id NOT IN (${placeholders})`, uniqueIds);
  ...
  await transaction(async () => {
    for (const id of existingIds) await deleteUserCascade(id, targetId);
  });

...
router.get('/recovery/summary', (req, res) => {
  const scope = scopeForUser(req.user);
  const summary = RECOVERY_BUCKETS.map((b) => ({
    key: b.key,
    label: b.label,
    count: countLeads(recoveryEligibleFilters({ ...scope, ...bucketFilters(b.key) })),
  }));
  res.json({ buckets: summary });
});

router.get('/recovery/leads', (req, res) => {
  const scope = scopeForUser(req.user);
  ...
  const rows = queryLeads(filters, { orderBy: 'leads.last_contact_date ASC' });
  res.json({
    leads: rows.map((l) => ({

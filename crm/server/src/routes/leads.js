...
async function leadTagIds(leadId) {
  const rows = await all('SELECT tag_id FROM lead_tags WHERE lead_id = ?', [leadId]);
  return rows.map((r) => r.tag_id);
}

async function logInteraction({ leadId, type, note, userId }) {
...
async function serializeLead(row) {
  if (!row) return null;
  return {
    id: row.id,
    name: row.name,
    ...
    tagIds: await leadTagIds(row.id),
  };
}
...
router.get('/leads', ah(async (req, res) => {
  ...
  const rows = await queryLeads(filters, { orderBy, limit: pageSize, offset: (page - 1) * pageSize });
  const total = await countLeads(filters);
  const leads = await Promise.all(rows.map(serializeLead));
  res.json({ leads, total, page, pageSize });
}));

// GET /api/leads/kanban -- all open leads grouped, ignoring pagination
router.get('/leads/kanban', ah(async (req, res) => {
  const scope = scopeForUser(req.user);
  const rows = await queryLeads(scope, { orderBy: 'leads.last_stage_change_at DESC' });
  const leads = await Promise.all(rows.map(serializeLead));
  res.json({ leads });
}));

router.get('/leads/:id', ah(async (req, res) => {
  const row = (await queryLeads({ ids: [req.params.id] }))[0];
  ...
  const lead = await serializeLead(row);
  const trialRows = await all(
  ...
  lead.trials = trialRows.map((t) => ({ id: t.id, status: t.status, date: t.date, time: t.time, teacherId: t.teacher_id, teacherName: t.teacher_name, levelIdentified: t.level_identified, objective: t.objective, teacherNotes: t.teacher_notes, result: t.result, createdAt: t.created_at, updatedAt: t.updated_at }));

  const proposalRows = await all('SELECT * FROM proposals WHERE lead_id = ? ORDER BY date DESC', [lead.id]);
  lead.proposals = proposalRows.map((p) => ({ id: p.id, date: p.date, packageId: p.package_id, packageLabel: p.package_label, value: p.value, paymentMethod: p.payment_method, specialCondition: p.special_condition, decisionDate: p.decision_date, status: p.status }));

  lead.enrollment = await one('SELECT * FROM enrollments WHERE lead_id = ? ORDER BY enrollment_date DESC LIMIT 1', [lead.id]);
  if (lead.enrollment) {
    lead.enrollment = {
  ...
  lead.student = await one('SELECT * FROM students WHERE lead_id = ?', [lead.id]);

  const noteRows = await all('SELECT * FROM notes WHERE lead_id = ? ORDER BY datetime DESC', [lead.id]);
  lead.notes_list = noteRows.map((n) => ({ id: n.id, text: n.text, userId: n.user_id, datetime: n.datetime }));

  const interactionRows = await all('SELECT * FROM interactions WHERE lead_id = ? ORDER BY datetime ASC', [lead.id]);
  lead.timeline = interactionRows.map((i) => ({ id: i.id, type: i.type, note: i.note, userId: i.user_id, datetime: i.datetime }));

  const taskRows = await all('SELECT * FROM tasks WHERE lead_id = ? ORDER BY due_date ASC', [lead.id]);
  lead.tasks = taskRows.map((t) => ({ id: t.id, title: t.title, type: t.type, dueDate: t.due_date, dueTime: t.due_time, assignedUserId: t.assigned_user_id, note: t.note, status: t.status }));

  res.json({ lead });
}));

router.post('/leads', requireRole('admin', 'manager', 'agent'), ah(async (req, res) => {
  const b = req.body || {};
  ...
  await logInteraction({ leadId: id, type: 'criacao', note: 'Lead cadastrado manualmente', userId: req.user.id });
  const row = (await queryLeads({ ids: [id] }))[0];
  res.status(201).json({ lead: await serializeLead(row) });
}));
...
router.put('/leads/:id', ah(async (req, res) => {
  ...
  if (!sets.length) {
    ...
    return res.json({ lead: await serializeLead(row) });
  }
  ...
  const row = (await queryLeads({ ids: [lead.id] }))[0];
  res.json({ lead: await serializeLead(row) });
}));
...
router.delete('/leads/:id', requireRole('admin', 'manager'), ah(async (req, res) => {
  const lead = await one('SELECT id FROM leads WHERE id = ?', [req.params.id]);
  if (!lead) return res.status(404).json({ error: 'Lead não encontrado' });
...
router.post('/leads/bulk-delete', requireRole('admin', 'manager'), ah(async (req, res) => {
  ...
  if (Array.isArray(ids) && ids.length) {
    ...
    const scope = scopeForUser(req.user);
    const rows = await queryLeads({ ...parseListFilters(filters), ...scope });
    targetIds = rows.map((r) => r.id);
  }
  if (targetIds.length === 0) return res.status(400).json({ error: 'Nenhum lead selecionado' });
  await transaction(async () => { for (const id of targetIds) await cascadeDeleteLead(id); });
  res.json({ ok: true, count: targetIds.length });
}));

// ---- stage transitions (Kanban) -----------------------------------------
router.post('/leads/:id/stage', ah(async (req, res) => {
  const lead = await one('SELECT * FROM leads WHERE id = ?', [req.params.id]);
  if (!lead) return res.status(404).json({ error: 'Lead não encontrado' });
...
// ---- notes ---------------------------------------------------------------
router.post('/leads/:id/notes', ah(async (req, res) => {
  const lead = await one('SELECT id FROM leads WHERE id = ?', [req.params.id]);
  if (!lead) return res.status(404).json({ error: 'Lead não encontrado' });
  ...
  const id = uid('note');
  const dt = nowISO();
  await run('INSERT INTO notes (id, lead_id, user_id, text, datetime) VALUES (?,?,?,?,?)', [id, lead.id, req.user.id, text, dt]);
  await logInteraction({ leadId: lead.id, type: 'nota', note: text, userId: req.user.id });
...
router.delete('/leads/:id/tags/:tagId', ah(async (req, res) => {
  await run('DELETE FROM lead_tags WHERE lead_id = ? AND tag_id = ?', [req.params.id, req.params.tagId]);
  res.json({ tagIds: await leadTagIds(req.params.id) });
}));

// ---- trial classes ---------------------------------------------------------
router.post('/leads/:id/trials', ah(async (req, res) => {
  const lead = await one('SELECT * FROM leads WHERE id = ?', [req.params.id]);
  if (!lead) return res.status(404).json({ error: 'Lead não encontrado' });
  const b = req.body || {};
  const id = uid('trl');
  ...
  if (b.teacherId && b.teacherId !== lead.teacher_id) {
    await run('UPDATE leads SET teacher_id = ? WHERE id = ?', [b.teacherId, lead.id]);
  }
  if (lead.status !== 'experimental_agendada' && (b.status === 'Agendada' || !b.status)) {
    await run('UPDATE leads SET status = ?, last_stage_change_at = ? WHERE id = ?', ['experimental_agendada', now, lead.id]);
  }
  await logInteraction({ leadId: lead.id, type: 'experimental', note: `Aula experimental agendada para ${b.date || 'data a definir'}`, userId: req.user.id });
...
router.put('/trials/:id', ah(async (req, res) => {
  const trial = await one('SELECT * FROM trial_classes WHERE id = ?', [req.params.id]);
  ...
  await run(`UPDATE trial_classes SET ${sets.join(', ')} WHERE id = ?`, params);

  const lead = await one('SELECT * FROM leads WHERE id = ?', [trial.lead_id]);
  if (b.status === 'Realizada' && lead) {
    await run('UPDATE leads SET status = ?, last_stage_change_at = ?, last_contact_date = ? WHERE id = ?', ['experimental_realizada', now, todayISO(), lead.id]);
    await logInteraction({ leadId: lead.id, type: 'experimental', note: `Aula experimental realizada${b.result ? ' — resultado: ' + b.result : ''}`, userId: req.user.id });
  } else if (b.status && lead) {
    await logInteraction({ leadId: lead.id, type: 'experimental', note: `Aula experimental atualizada: ${b.status}`, userId: req.user.id });
  }
  res.json({ ok: true });
}));

// ---- proposals ---------------------------------------------------------------
router.post('/leads/:id/proposals', requireRole('admin', 'manager', 'agent'), ah(async (req, res) => {
  const lead = await one('SELECT * FROM leads WHERE id = ?', [req.params.id]);
  if (!lead) return res.status(404).json({ error: 'Lead não encontrado' });
  const b = req.body || {};
  const id = uid('prp');
  ...
  await logInteraction({ leadId: lead.id, type: 'proposta', note: `Proposta enviada${b.packageLabel ? ' — ' + b.packageLabel : ''}`, userId: req.user.id });
  res.status(201).json({ proposal: { id, leadId: lead.id, ...b } });
}));

router.put('/proposals/:id', ah(async (req, res) => {
  const proposal = await one('SELECT * FROM proposals WHERE id = ?', [req.params.id]);
  ...
  await run(`UPDATE proposals SET ${sets.join(', ')} WHERE id = ?`, params);
  if (b.status) {
    await logInteraction({ leadId: proposal.lead_id, type: 'proposta', note: `Status da proposta atualizado para "${b.status}"`, userId: req.user.id });
    if (b.status === 'Em negociação') {
      await run('UPDATE leads SET status = ?, last_stage_change_at = ? WHERE id = ?', ['negociacao', now, proposal.lead_id]);
    } else if (b.status === 'Recusada' || b.status === 'Sem resposta') {
      await run('UPDATE leads SET status = ?, last_stage_change_at = ? WHERE id = ?', ['recuperacao', now, proposal.lead_id]);
    }
  }
  res.json({ ok: true });
}));
...
router.post('/leads/:id/enroll', requireRole('admin', 'manager', 'agent'), ah(async (req, res) => {
  const lead = await one('SELECT * FROM leads WHERE id = ?', [req.params.id]);
  if (!lead) return res.status(404).json({ error: 'Lead não encontrado' });
  const b = req.body || {};
  const now = nowISO();
  ...
  await logInteraction({ leadId: lead.id, type: 'matricula', note: 'Matrícula confirmada', userId: req.user.id });
